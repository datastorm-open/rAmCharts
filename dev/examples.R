...
pipeR::pipeline(
  amSerialChart(theme = "patterns", categoryField = "name", creditsPosition = "top-right",
                path = system.file("htmlwidgets/lib", package = "rAmCharts")),
  setDataProvider(data.frame(name = paste0("foo", 1:5),
                             random1 = round(rnorm(5, mean = 5)),
                             random2 = round(rnorm(5, mean = 3)))),
  addGraph(type = "column", valueField = "random1"),
  addGraph(type = "column", valueField = "random2"),
  addListener("clickGraphItem" , "function(event) {alert('ok !');}"), 
  setChartCursor(),
  setChartScrollbar(),
  plot(),
  htmlwidgets::saveWidget("foo.html")
)
...
htmlwidgets::saveWidget(widget = obj, file = "toto", libdir = "foo")

###  AmGanttChart

pipeR::pipeline(
  amGanttChart(brightnessStep = 20, rotate = TRUE, theme = "dark",
               startField = "start", endField = "end", segmentsField = "segments",
               dataDateFormat = "YYYY-MM-DD", startDate = "2015-01-01",
               period = "hh", categoryField = "category", durationField = "duration",
               valueAxis = valueAxis(type = "date"), graph = amGraph(fillAlphas = 1)),
  setDataProvider(data.table(category = c("John", "Julia"))),
  addSegment(1:2, list(data.table(start = 7, duration = 2:3, task = c("Task #1", "Task #2")), 
                       data.table(start = 10, duration = 2:3, task = c("Task #1", "Task #2")))),
  plot()
)
...
pipeR::pipeline(
  ...
  addSubData(1, data.table(modality = c("3G", "4G"),
                           utility = c(-1,2),
                           color = c("#007FFF", "#007FFF"))),
  setSubChartProperties(type = "serial", creditsPosition = "bottom-right",
...
pipeR::pipeline(
  ...
  addTitle(text = "TEST"),
  setDataProvider(data.table(binf = c(1,2,3,4,5), bsup = c(3,4,5,6,7),
                             value = c(2,3,4,5,6), heure = c("9h","10h","11h","12h","13h"))),
  ...
  setGraphs(list(amGraph(id = "2014_from", lineAlpha = 0, valueField = "binf",
                         showBalloon = FALSE, visibleInLegend = FALSE),
                 amGraph(id = "2014_to", lineAlpha = 0, fillAlphas = 0.2,
  ...
  setLegend(amLegend = amLegend(equalWidths=FALSE, position="bottom", valueAlign="left", valueWidth=100) %>>%
              ...
              addListener(name = "showItem" , 'function(event){
                          var id = event.dataItem.id;
...
# ---------------------------------
# Unreferenced Stacked Column Chart
# ---------------------------------
...
amSerialChart(categoryField = "date", theme = "light", startDuration = 0,
...
# ---------------------------------------
# Unreferenced AmSerialChart: mixed dates
# ---------------------------------------
